...
try {
    ...
    $sql_room_detail = 'SELECT room_id, room_detail.* FROM room_detail WHERE delete_flg = 0';
    $stmt = $model->dbh->query($sql_room_detail);
                  ...
                  <?php foreach ($room_details as $k => $v):?>
                    <?php foreach ($v as $va):?>
                            <?php if ($value['id'] == $va['room_id']):?>
                                <?=$va['capacity']?>名様：￥<?=$va['price']?><?=$va['remarks']?><br>
                        <?php endif?>
                    <?php endforeach;?>
                  <?php endforeach;?>
              </p>
